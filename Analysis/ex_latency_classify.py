...
def packet_ho_classify(hodf, lossdf, recvdf, fout, secs=1, overlap=True, ratio=1):
    ...
    recvdf = recvdf[recvdf['latency'] > 100e-3]
    excslatency_timestamps = recvdf['Timestamp'].array
    for i, item in enumerate(all_intervals):
        ...
        for j in range(len(excslatency_timestamps)):
            if item.empty:
                break
            # if lossdf['Timestamp'].iloc[j] > item.upper:
            if excslatency_timestamps[j] > item.upper:
                break
            # if lossdf['Timestamp'].iloc[j] in item:
            if excslatency_timestamps[j] in item:
                sum_loss_num[i] += 1
    ...
    for _exp, (_times, _rounds) in Exp_Name.items():
        ...
        for j in range(_times):
            for i, dev in enumerate(devices):
                ...
                makedir(os.path.join(dirpath, "udp-excslat-classify-50p"))  # 0.5 == 50 percent
                ...
                fout = os.path.join(dirpath, "udp-excslat-classify-50p", "dwnlnk_ho_statistics.csv")
                ho_statistics(lossdf, recvdf, hodf, fout)
                for i in tqdm(range(10)):
                    fout = os.path.join(dirpath, "udp-excslat-classify-50p", "dwnlnk_excslat_ho_classify_0{}.csv".format(i))
                ...
                for i in tqdm(range(1, 11)):
                    fout = os.path.join(dirpath, "udp-excslat-classify-50p", "dwnlnk_excslat_ho_classify_{}.csv".format(i))
        ...
        for j in range(_times):
            for i, dev in enumerate(devices):
                ...
                fout = os.path.join(dirpath, "udp-excslat-classify-50p", "uplnk_ho_statistics.csv")
                ho_statistics(lossdf, recvdf, hodf, fout)
                for i in tqdm(range(10)):
                    fout = os.path.join(dirpath, "udp-excslat-classify-50p", "uplnk_excslat_ho_classify_0{}.csv".format(i))
                    packet_ho_classify(hodf.copy(), lossdf.copy(), recvdf.copy(), fout, secs=i/10, overlap=False, ratio=0.5)
                for i in tqdm(range(1, 11)):
                    fout = os.path.join(dirpath, "udp-excslat-classify-50p", "uplnk_excslat_ho_classify_{}.csv".format(i))
                    packet_ho_classify(hodf.copy(), lossdf.copy(), recvdf.copy(), fout, secs=i, overlap=False, ratio=0.5)
                print()
