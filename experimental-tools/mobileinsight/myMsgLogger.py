...
class MyMsgLogger(Analyzer):
    ...
    def save_decoded_msg_as(self, filepath):
        ...
        try:
            ...
            self._save_file = open(self._save_file_path, 'a')
    ...
    def __dump_message(self, msg):
        ...
        if self.decode_type == self.XML:
            root = ET.fromstring(msg.data.decode_xml())
            new_pair = ET.Element('pair')
            new_pair.set('key', 'device_timestamp')
            new_pair.text = date
            root.append(new_pair)
            decoded_msg = ET.tostring(root).decode()
